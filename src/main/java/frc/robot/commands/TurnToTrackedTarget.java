...
public class TurnToTrackedTarget extends CommandBase {
    ...
    public void execute() {
      if (vision.hasTarget()) {
        double visionError = vision.getBestTarget().get().getYaw();
        double value = -Math.min(visionError*Constants.TRACK_TAG_ROTATION_KP, 1);  //calculate motor percentage value
    ...
    public boolean isFinished() {
        // return Math.abs(visionError < 3.0);
        return false;
    }
}
